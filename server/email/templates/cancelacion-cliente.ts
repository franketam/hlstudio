import { fechaLargaAR, horaCortaAR } from "@/lib/format";
import type { RenderedEmail } from "./confirmacion-cliente";

export type CancelacionClienteData = {
...
export function renderCancelacionCliente(
  data: CancelacionClienteData
): RenderedEmail {
  const fecha = fechaLargaAR(data.inicio);
  const hora = horaCortaAR(data.inicio);

  const subject = `Tu turno fue cancelado — ${fecha}, ${hora} hs`;

  const text = [
    `Hola ${data.clienteNombre},`,
    ``,
    `Te avisamos que tu turno en HLstudio fue cancelado.`,
    ``,
          ...
          <tr>
            <td style="padding:28px 32px 8px 32px;">
              <p style="margin:0 0 20px 0;font-size:15px;line-height:1.55;color:#2a2a26;">
                Hola ${escapeHtml(data.clienteNombre)}, te avisamos que el siguiente turno fue cancelado:
              </p>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:1px solid #ececea;">
                ...
                ${row("Servicio", escapeHtml(data.servicioNombre))}
              </table>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:24px 32px 32px 32px;">
              <p style="margin:0 0 14px 0;font-size:14px;line-height:1.5;color:#2a2a26;">
                Si querés reservar otro horario, podés hacerlo desde acá:
              </p>
              <p style="margin:0;">
                <a href="${escapeHtml(data.reservarUrl)}" style="display:inline-block;padding:12px 22px;background:#0a0a0a;color:#ffffff;text-decoration:none;font-size:14px;letter-spacing:0.04em;border-radius:2px;">
                  Reservar otro turno
                </a>
              </p>
              <p style="margin:18px 0 0 0;font-size:13px;line-height:1.5;color:#8a8a86;">
                Disculpá las molestias.
              </p>
            </td>
          </tr>
