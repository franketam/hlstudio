...
export type CancelacionBarberoData = {
...
export function renderCancelacionBarbero(
  data: CancelacionBarberoData
): RenderedEmail {
  const fecha = fechaLargaAR(data.inicio);
  const hora = horaCortaAR(data.inicio);

  const subject = `Turno cancelado — ${fecha}, ${hora} hs`;
          ...
          <tr>
            <td style="padding:28px 32px 8px 32px;">
              <p style="margin:0 0 18px 0;font-size:15px;line-height:1.55;color:#2a2a26;">
                Hola ${escapeHtml(data.barberoNombre)}, se canceló este turno:
              </p>
